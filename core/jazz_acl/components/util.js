...
const createRule = (policies, config) => {
  ...
  for (let key in userPolicies) {
    ...
    const managePolicy = userPolicy.filter(policy => policy.category === 'manage');
    const codePolicy = userPolicy.filter(policy => policy.category === 'code');
    const deployPolicy = userPolicy.filter(policy => policy.category === 'deploy');

    if (manageAdminPolicy.length > 0) {
      let adminPolicies = config.MANAGE_ADMIN;
      adminPolicies = adminPolicies.map(policy => { return { permission: policy.permission, category: policy.category, userId: key } });
      newPolicies = newPolicies.concat(adminPolicies);
    } else if ((codePolicy.length > 0 || deployPolicy.length > 0)) {
      let readPolicy = config.READ;
      readPolicy = readPolicy.map(policy => { return { permission: policy.permission, category: policy.category, userId: key } });
      ...
      manageReadPolicy = manageReadPolicy.map(policy => { return { permission: policy.permission, category: policy.category, userId: key } });
