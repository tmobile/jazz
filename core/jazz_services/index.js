...
module.exports.handler = (event, context, cb) => {
  ...
  var handleResponse = function(error, data, input) {
    if (error) {
      ...
      if (error.result === "inputError") {
        ...
        cb(
          JSON.stringify(
            errorHandler.throwInternalServerError("unexpected error occured")
          )
        )
      }
    } else {
  ...
  try {
    ...
    if (!event.method) {
      return cb(
        JSON.stringify(errorHandler.throwInputValidationError("method cannot be empty"))
      )
    }
    ...
    if (!event.principalId) {
      return cb(
        JSON.stringify(
          errorHandler.throwUnauthorizedError(
            "You aren't authorized to access this service. Please login with your credentials."
          )
        )
      )
    }
    ...
    if ((event.method === "PUT" || event.method === "DELETE") && !service_id) {
      return cb(
        JSON.stringify(errorHandler.throwInputValidationError("service id is required"))
      )
    }
      ...
      async.series(
        {
          ...
          getServiceByServiceId: function(onComplete) {
            validateUtils.validateServiceWithServiceId(service_id, function onValidate(
              error,
              data
            ) {
      ...
      async.series(
        {
          validate: function(callback) {
            validateUtils.validateUpdatePayload(
              service_id,
              update_data,
              function onValidate(error, data) {
                old_data = data
                callback(error, data)
              }
            )
        ...
        function(error, data) {
          ...
          if (error) {
            ...
            return handleResponse(
              error,
              {
                message: "Successfully updated service with id: " + service_id,
    ...
    if (event.method === "POST" && !service_id) {
      ...
      logger.info(
        "Create a new service with the following payload data : " +
      ...
      async.series(
        {
          ...
          validate: function(callback) {
            validateUtils.validateCreatePayload(service_data, function onValidate(
              error,
              data
            ) {
              callback(error, data)
    ...
    logger.error(e)
    return cb(
      JSON.stringify(errorHandler.throwInternalServerError("Unexpected error occured"))
    )
  }
}
