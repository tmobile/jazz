...
func (writer *logWriter) Write(bytes []byte) (int, error) {
	return fmt.Print(time.Now().UTC().Format("2006-01-02T15:04:05.999Z") + "" + writer.levelName + " \t " + config["requestDetails"] + "" + string(bytes))
}
...
func (l *Logger) Init(level string, ctx context.Context) {
	...
	config["requestDetails"] = lc.AwsRequestID
}
