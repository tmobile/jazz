            ...
            async.series({
                ...
                validateInputData: function(onComplete) {
                    ...
                    if (invalid_fields.length > 0) {
                        cb(JSON.stringify(errorHandler.throwInputValidationError("Invalid field(s) - " + invalid_fields.join(", ") + ". Only following fields can be updated - " + fields_list.join(", "))));
                    }
...
logger.info('aaa4444444');
                    ...
                    if (invalid_fields.length > 0) {
                        onComplete(null, {
                            "result": "warning",
                            "message": "Invalid field(s) - " + invalid_fields.join(", ") + ". Only following fields can be updated - " + allowed_fields.join(", ")
                        });
                    } else {
                    ...
                    crud.getList({ 'service': service_data.service, 'domain': service_data.domain }, function onServiceGet(error, data) {
                        if (error) {
                            ...
                            if (data.length > 0) {
                                ...
                                onComplete(null, {
                                    "result": "success",
                                    "message": "Valid service_name - domain_name combination"
                                });
                            }
                        }
                    });
                ...
                // Handle error
                if (error) {
                    logger.error('error occured while adding new service'+ JSON.stringify(error));
                    logger.error(error);
